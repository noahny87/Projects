...
library(plotly)
library(gapminder)
setwd("C:/Users/ECYA1QY/OneDrive - Deere & Co/Documents/Rprogrammincourse2")
#load in data set  
...
### Age Blood Pressure and Heart Disease cor. 
```{r Age vs Blood Pressure vs Heart Disease,echo=FALSE,message=FALSE}
#Lets make a graph graphing the age vs blood pressure in a scatter plot and have them be filled in by Heart Disease Y or N 
plott <- ggplot(data = heart_dis_df, aes(RestingBP, fill = HDYN)) +
  geom_point(position = "stack", stat = "count", aes(text = paste("Age: ", Age, "<br>Resting BP: ", RestingBP)), size = 2.5) +
  labs(title = "Age vs BloodPressure- HD cor.", xlab = "RestingBP", ylab = "Count") +
  scale_fill_manual(values = c("Y" = "yellow", "N" = "blue"))

ggplotly(plott)

```

There is a weak correlation between age, blood pressure and if there is heart disease or not. At the top of each line is where the younger ages are and its noticeable more blue is around that area meaning less heart disease. 
